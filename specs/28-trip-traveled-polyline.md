...
5. **Tests del service y del endpoint.** En `tests/Unit/TripServiceTest.php` y `tests/Feature/TripTest.php`: finish con tres puntos → columna codificada y `traveledPoints` con los tres pares en orden; finish sin puntos → `null` y `[]`; segundo finish tras un `PATCH` de admin que devuelve el viaje a `in_route` → se recodifica incluyendo los puntos nuevos; el `PATCH` general con `traveledPolyline` en el body no la toca.
...
- [ ] Un segundo `/finish` (tras un `PATCH` de admin que devuelve el viaje a `in_route`) sobrescribe `traveled_polyline` con el rastro completo, incluidos los puntos nuevos.
