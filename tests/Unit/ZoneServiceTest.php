...
/*
|--------------------------------------------------------------------------
| Resolución punto → zona
|--------------------------------------------------------------------------
*/

it('devuelve la zona activa que contiene el punto', function () {
    $zone = Zone::factory()->withArea(zoneSquare(14.0, -91.0))->active()->create();
    Zone::factory()->withArea(zoneSquare(0.0, 0.0))->active()->create();

    expect(zoneService()->getZoneContainingPoint(14.5, -90.5)->id)->toBe($zone->id);
});

it('devuelve null cuando el punto no cae en ninguna zona', function () {
    Zone::factory()->withArea(zoneSquare(14.0, -91.0))->active()->create();

    expect(zoneService()->getZoneContainingPoint(-33.0, 18.0))->toBeNull();
});

it('devuelve null cuando la única zona que contiene el punto está inactiva', function () {
    Zone::factory()->withArea(zoneSquare(14.0, -91.0))->inactive()->create();

    expect(zoneService()->getZoneContainingPoint(14.5, -90.5))->toBeNull();
});

it('elige la zona de menor id cuando el punto cae en un solape', function () {
    $first = Zone::factory()->withArea(zoneSquare(14.0, -91.0))->active()->create();
    Zone::factory()->withArea(zoneSquare(14.2, -90.8))->active()->create();

    expect(zoneService()->getZoneContainingPoint(14.5, -90.5)->id)->toBe($first->id);
});

it('ignora las zonas inactivas al resolver el punto y devuelve la activa', function () {
    Zone::factory()->withArea(zoneSquare(14.0, -91.0))->inactive()->create();
    $active = Zone::factory()->withArea(zoneSquare(14.0, -91.0))->active()->create();

    expect(zoneService()->getZoneContainingPoint(14.5, -90.5)->id)->toBe($active->id);
});
