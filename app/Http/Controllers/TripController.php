...
class TripController extends Controller
{
    ...
    #[OA\Patch(
        ...
        DESDE SPEC 31 EL CUERPO ADMITE ADEMÁS DOS CAMPOS OPCIONALES —expenseAmount y expenseDescription—, el primer viático del viaje. Si viaja expenseAmount se inserta una fila en trip_expenses en la MISMA transacción, sin confirmar; si no viaja, no se crea nada y la asignación es exactamente la de SPEC 27. expenseDescription sin expenseAmount se ignora en silencio. Reasignar con monto AÑADE otro viático. Nada de esto bloquea /start.
        ...
        responses: [
            ...
            new OA\Response(
                response: 422,
                description: 'Datos inválidos, con el formato propio de Laravel {message, errors}. Casos: falta cualquiera de los CUATRO campos (El piloto es obligatorio / El vehículo es obligatorio / Los galones de combustible son obligatorios / El tipo de combustible es obligatorio); se envía NULL en cualquiera de ellos, porque LA DESASIGNACIÓN NO EXISTE; el id NO EXISTE en su tabla (El piloto seleccionado no existe / El vehículo seleccionado no existe); fuelGallons no es numérico, es 0 o es negativo (Los galones de combustible deben ser un número / Los galones de combustible deben ser mayores a 0); o fuelType queda fuera de los cuatro casos del enum (El tipo de combustible seleccionado no es válido); o, si viajan, expenseAmount no es numérico, es 0, es negativo o supera 99999999.99 (El monto del viático debe ser un número / El monto del viático debe ser mayor a 0 / El monto del viático no puede superar 99999999.99) y expenseDescription no es texto o supera 255 caracteres (La descripción del viático debe ser texto / La descripción del viático no puede superar los 255 caracteres). ATENCIÓN — ES EL 422 QUE ROMPE A LOS CLIENTES ANTERIORES A SPEC 27: un cuerpo con solo pilotId y vehicleId cae aquí con los dos mensajes de combustible. Y un piloto sin rol de piloto, un piloto sin empresa, un vehículo no activo o una pareja de empresas distintas NO caen aquí: son 400.',
                content: new OA\JsonContent(ref: '#/components/schemas/ValidationError'),
            ),
        ],
    )]
