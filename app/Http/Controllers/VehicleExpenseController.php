...
#[OA\Tag(
    ...
    description: 'Gastos de mantenimiento imputados a un vehículo: llantas, frenos, aceite, mano de obra y otras 18 categorías, cada una con su naturaleza preventiva o correctiva, su monto en quetzales, su fecha y su descripción. Todos los endpoints requieren token JWT (Authorization: Bearer {token}). ATENCIÓN — el recurso cuelga de un vehículo pero la ruta NO está anidada: no existe /api/vehicles/{vehicle}/expenses; el vínculo viaja en el campo vehicle_id del cuerpo y en el query param vehicleId del listado, que es OBLIGATORIO. NO EXISTE UN LISTADO GLOBAL de los gastos de toda la flota, ni comparativas entre vehículos, ni reportes por categoría o por mes: el único agregado del dominio es el totalAmount del listado de un vehículo. Este dominio NO lleva el middleware carrier.required, a diferencia de Vehicles: el ámbito lo comprueba el servicio a partir del vehículo, y un carrier sin empresa recibe 403 con el mensaje No perteneces a ninguna empresa transportista. Los permisos van por acción: leer (index y show) es cosa de carrier, administrator y manager; escribir (store, update y destroy) solo de carrier y administrator, así que un manager recibe 403 en los tres. Un pilot recibe 403 en los cinco. El ámbito lo fija el rol: un carrier solo alcanza los vehículos de su propia empresa —tocar uno ajeno es 403, no 404— y administrator y manager alcanzan los de todas. El estado del vehículo no importa: uno inactive acepta y lista gastos igual que uno active. GET /api/vehicles/{vehicle} no cambió con este dominio: no devuelve expenses ni totales, y los gastos se piden siempre aparte.',
)]
class VehicleExpenseController extends Controller
{
    ...
    #[OA\Post(
        ...
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(ref: '#/components/schemas/StoreVehicleExpenseRequest'),
        ),
        security: [['bearerAuth' => []]],
        tags: ['VehicleExpenses'],
        responses: [
            ...
            new OA\Response(
                response: 422,
                description: 'Datos inválidos: falta alguno de los SEIS campos obligatorios, el vehículo no es un entero, la categoría o la naturaleza no pertenecen a su enum, el monto no es numérico, no llega a 0.01 o supera 99999999.99, la fecha no es válida o es futura, o la descripción supera los 1000 caracteres',
                content: new OA\JsonContent(ref: '#/components/schemas/ValidationError'),
            ),
        ],
    )]
