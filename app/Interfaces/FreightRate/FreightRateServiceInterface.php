...
interface FreightRateServiceInterface
{
     ...
     * collection always comes back. Deleted rates are out of it. The location, the
     * product and the registering user travel eager loaded, so labelling the rows costs
     * no extra query per row.
     *
     * @param  array{locationId?: string|null}  $filters
     *                                                    locationId: narrows the table to a single location. A non numeric value is
     *                                                    ignored instead of failing, and a numeric one matching no location yields an empty list.
     ...
     *
     * The location and the product must both be active: quoting a pair that cannot be
     * quoted is refused with a BadRequestError. The band is refused with the same
     * error when a live rate already holds that same combination of location, product,
     ...
     * its fuel_min again.
     *
     * @param  array{locationId: int, productId: int, fuelType: string, fuelMin: float, pricePerPound: float}  $data
     ...
     * pointing at whoever registered the rate. The location and the product are checked
     * to be active even when the payload only moves the price — a rate is not edited while
     ...
     * of locations and products it is NOT idempotent — a second call surfaces the guard's
     * BadRequestError, which is information and not a client mistake. It works on a
     * rate whose location or product went inactive: deleting is never blocked.
     */
    public function destroy(int $id): FreightRate;

    /**
     * Resolve the rate applying to the given destination and return the quote.
     *
     ...
     *
     * The destination is addressed by id and checked to be active, the product is
     * checked to be active, and among the live bands of that location, product and fuel
     * type the one with the highest fuel_min not above the current fuel price wins;
     * when the fuel is cheaper than every band, the lowest one does. Band selection
     * itself never fails, and the whole method has no 404: a location id that does not
     * exist was already rejected as a 422 by the form request.
     *
     * Throws a BadRequestError — each with its own message — when the location is
     * inactive, when the product is inactive, when the fuel type has no price in
     * effect, or when the trio has no rate quoted at all.
     *
     * @param  array{locationId: int, productId: int, fuelType: string, pounds?: float|null}  $filters
     ...
     *                                                                                                   rate: the applied band, with its location and product loaded;
