...
- **Matriz de roles** (reestructura posterior a SPEC 33, sin spec; roles explícitos en cada `routes/*.php`, **sin bypass implícito** del administrador en `EnsureUserHasRole`):
  ...
  - `export` — `POST/PATCH/DELETE /trips` (la tripulación la sigue ignorando el `PATCH`), CRUD de `clients`, `shipping-lines`, `finished-products`, `locations` y `departure-points` (con `toggle-status`), `dashboard` y `assistant` sin ámbito de empresa, y lectura de `vehicles` y `pilots` (index) de todas las empresas. No lee `vehicle-expenses` ni `carriers`: la tool `vehicle_expenses` del asistente le devuelve `{"error"}`.
  - `user` — **solo viajes en lectura**: listado, detalle, `/positions`, `/timeouts`, `/fuels`, `/expenses`, `/cost` y el canal, de **cualquier** viaje, más la lectura de `finished-products` (SPEC 36). Todo lo demás, 403.
