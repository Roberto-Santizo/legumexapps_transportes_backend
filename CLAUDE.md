...
En un `apiResource`, las rutas fijas (`/join`, `/me`, `/me/pilots`, `/current`, `/quote`, `/directions`, `/{pilot}/salary`, `/{trip}/assignment`, `/{trip}/start`, `/{trip}/finish`, `/{trip}/positions`, `/{trip}/fuels`, `/{trip}/timeouts`) se declaran **antes** del resource: si no, las captura el comodín `{carrier}`.

**Las únicas rutas anidadas del proyecto cuelgan de `/api/trips/{trip}/…`** (`/positions` desde SPEC 26, `/fuels` y `/timeouts` desde SPEC 27): SPEC 14 y SPEC 18 evitaron a propósito `/api/vehicles/{vehicle}/expenses` y `/api/accessories/{accessory}/characteristics`, pero una posición, una carga o una parada sin su viaje no significan nada y `{trip}` ya era el parámetro del grupo. Viven en `routes/trips.php`, que por eso —y por el `/current`— declara **catorce** rutas y no ocho. La excepción declarada es `PATCH /api/trip-fuels/{tripFuel}/confirm`, en `routes/trip-fuels.php`: el id de la carga ya identifica el viaje, así que no se anida.

Dominios ya implementados: `Auth`, `Carrier`, `Vehicle`, `VehicleExpense`, `FuelPrice`, `Product`, `Zone`, `Location`, `FreightRate`, `Pilot`, `Place`, `Accessory`, `AccessoryCharacteristic`, `DeparturePoint`, `Client`, `ShippingLine`, `Trip`, `TripPosition`, `TripFuel`, `TripTimeout`, `Dashboard` (SPEC 01–29; **hay dos SPEC 27**, `27-trip-fuels.md` y `27-trip-timeouts.md`, ambas implementadas; SPEC 28 no crea dominio, amplía `Trip`; SPEC 29 crea `Dashboard` sin tabla ni FormRequest).
...
- 19 columnas de negocio (20 con la `traveled_polyline` de SPEC 28) + `timestamps` + `deleted_at`; `polyline`, `traveled_polyline` y `observations` son `TEXT`. Enum `App\Enums\TripStatus` con **tres** casos (`pending`, `in_route`, `finished`) y **sin `cancelled`**: un viaje que no se hará se borra. El `status` nace en `pending`, no se acepta en el `POST` y sale con el valor crudo del enum en inglés.
- **El `PATCH` deja de ser el único camino de escritura**: tres rutas fijas de acción, cada una con su rol y su efecto, declaradas antes del `apiResource`.
  ...
  |---|---|---|
  | `PATCH /{trip}/assignment` | `role:carrier` + `carrier.required` | Fija `pilot_id`, `vehicle_id` y `assigned_by`; **no toca `status`**. Desde SPEC 27 crea además la **primera carga de combustible** (`fuelGallons`, `fuelType`) en la misma transacción |
  ...
  | `PATCH /{trip}/finish` | `role:pilot` (el asignado) | `end_date = now()`, `status = finished`. Desde SPEC 27 **cierra la parada abierta** del viaje, si la hay; desde SPEC 28 **codifica el rastro** en `traveled_polyline` en el mismo `update()` |
...
- **Dos Resources**, a diferencia del resto del proyecto: `TripResource` (**37 claves** desde SPEC 28 —las 34 de SPEC 24, `totalFuelGallons` de SPEC 27 y `traveledPolyline`/`traveledPoints` de SPEC 28—, ids planos con su nombre al lado y `points` decodificados por el `PolylineDecoder` de SPEC 16) en los siete endpoints restantes, y `TripListResource` (**15 claves**) solo en el listado — sin ids de relación, sin `polyline` ni `points`, sin imágenes y sin las tres marcas de tiempo, para que un `limit=100` no decodifique cien polilíneas. El detalle carga ocho relaciones (con `pilot.pilotDocument`) más el `withSum`/`loadSum` de combustible; el listado, seis (sin `client` ni `assignedBy`).
...
- **Tabla append-only**: sin `PATCH` de galones, sin `DELETE`, sin desconfirmar, sin cantidad recibida ni observaciones. No hay `GET /api/trip-fuels` global, ni websocket, ni filtro de combustible en `GET /api/trips`; `TripListResource` sigue en 15 claves. `/finish` **no** exige nada de combustible.
...
- `TripResource` pasa de 35 a **37 claves**: `traveledPolyline` (string o `null`) y `traveledPoints` (pares decodificados, `[]` si `null`), insertadas entre `points` y `observations`. `TripListResource` sigue en 15, `TripPositionResource` en 5 y el payload de `TripPositionUpdated` en 6.
...
- **`/vehicles`**: **todos** los vehículos, incluidos `inactive`, orden `id ASC`; filtros tolerantes extra `status`, `condition` e `inRoute` (`FILTER_VALIDATE_BOOLEAN`, aplicado **antes** de paginar para que `total` sea correcto). 11 claves, **sin `purchasePrice` ni `monthlyInsuranceCost`**. `currentTrip` (y `inRoute`) = el `in_route` de `start_date desc, id desc`, o `null`/`false`.
...
- Fuera, si llega, en otra spec: tablero por empresa, combustible agregado, empresas y pilotos, paradas históricas, inventario de accesorios, Top N, comparativas, export, caché, jobs y websocket.
