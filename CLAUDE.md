...
**Las únicas rutas anidadas del proyecto cuelgan de `/api/trips/{trip}/…`** (`/positions` desde SPEC 26, `/fuels` y `/timeouts` desde SPEC 27, `/expenses` desde SPEC 31, `/cost` desde SPEC 33): SPEC 14 y SPEC 18 evitaron a propósito `/api/vehicles/{vehicle}/expenses` y `/api/accessories/{accessory}/characteristics`, pero una posición, una carga, una parada, un viático o el costo sin su viaje no significan nada y `{trip}` ya era el parámetro del grupo. Viven en `routes/trips.php`, que por eso —y por el `/current`— declara **diecisiete** rutas y no ocho. Las excepciones declaradas son `PATCH /api/trip-fuels/{tripFuel}/confirm` (`routes/trip-fuels.php`) y `PATCH /api/trip-expenses/{tripExpense}/confirm` (`routes/trip-expenses.php`): el id de la carga o del viático ya identifica el viaje, así que no se anida. `/api/trip-finished-products` (SPEC 37) tampoco se anida, por decisión explícita: el viaje viaja en el query param obligatorio `tripId` y en el body, como `vehicle-expenses`.

Dominios ya implementados: `Auth`, `Carrier`, `Vehicle`, `VehicleExpense`, `FuelPrice`, `Product`, `Zone`, `Location`, `FreightRate`, `Pilot`, `Place`, `Accessory`, `AccessoryCharacteristic`, `DeparturePoint`, `Client`, `ShippingLine`, `Trip`, `TripPosition`, `TripFuel`, `TripTimeout`, `Dashboard`, `TripExpense`, `TripCost`, `DeviceToken`, `PushNotification`, `TripNotification`, `FinishedProduct`, `TripFinishedProduct` (SPEC 01–38; **hay dos SPEC 27**, `27-trip-fuels.md` y `27-trip-timeouts.md`, ambas implementadas; SPEC 28, SPEC 30 y SPEC 32 no crean dominio, amplían `Trip`; SPEC 29 crea `Dashboard` sin tabla ni FormRequest; SPEC 31 crea `TripExpense` como calco de `TripFuel`; SPEC 33 crea `TripCost` sin tabla, sin modelo y sin FormRequest; SPEC 34 crea `DeviceToken`, que guarda datos para una capacidad —el envío push— que SPEC 35 añade después con `PushNotification` (contrato por capacidad, sin HTTP) y `TripNotification` (sin tabla, sin ruta y sin FormRequest); SPEC 36 crea `FinishedProduct`, catálogo sin relación alguna con `Product`; SPEC 37 crea `TripFinishedProduct`, las líneas de productos terminados del viaje, con rutas sin anidar; SPEC 38 no crea dominio: da a `Report` su primera capa HTTP con `GET /api/reports/trips`). `Assistant` y `Report` nacieron **sin spec** en la rama `feat/ai-integration`: el contrato del asistente y de las dos exportaciones de la tool vive solo en esta guía y en el Swagger; el reporte descargable sí tiene spec (SPEC 38). `Report` (`app/Interfaces/Report/`, `app/Services/Report/`, `ReportProvider`) fue el único dominio sin capa HTTP hasta SPEC 38, que le añade `ReportController` y `ExportTripsReportRequest` **sin Resource**, porque la respuesta no es JSON.
...
- **Matriz de roles** (reestructura posterior a SPEC 33, sin spec; roles explícitos en cada `routes/*.php`, **sin bypass implícito** del administrador en `EnsureUserHasRole`):
  ...
  - `export` — `POST/PATCH/DELETE /trips` (la tripulación la sigue ignorando el `PATCH`), escritura de `trip-finished-products` (SPEC 37, junto al `administrator`), CRUD de `clients`, `shipping-lines`, `finished-products`, `locations` y `departure-points` (con `toggle-status`), `dashboard` y `assistant` sin ámbito de empresa, y lectura de `vehicles` y `pilots` (index) de todas las empresas. No lee `vehicle-expenses` ni `carriers`: la tool `vehicle_expenses` del asistente le devuelve `{"error"}`.
  - `user` — **solo viajes en lectura**: listado, detalle, `/positions`, `/timeouts`, `/fuels`, `/expenses`, `/cost` y el canal, de **cualquier** viaje, más la lectura de `finished-products` (SPEC 36) y de `trip-finished-products` (SPEC 37). Todo lo demás, 403.
  ...
  - Ámbito: `TripService::UNSCOPED_ROLES` (todos salvo `carrier` y `pilot`) ve todos los viajes; `VehicleService`, `PilotService` y `DashboardService` añaden `export` a administrator/manager. Catálogos, `places`, `fuel-prices` y `freight-rates` leen con `role:` + `UserRole::allExcept(UserRole::User, UserRole::Shipment)` a nivel de grupo —salvo `finished-products` (SPEC 36), que lee con `UserRole::allExcept(UserRole::Pilot)`: el primer catálogo abierto a `user` y `shipment` y cerrado al `pilot`— — por eso sus tests de N+1 aceptan **tres** consultas a `users` (el `role:` recarga el usuario por el guard `api`, porque `jwt.auth` autentica contra el guard por defecto). `tests/Feature/RoleAccessTest.php` cubre la matriz entre dominios.
...
## Catálogos nacionales (fuel prices, products, zones, locations, departure points, freight rates, clients, shipping lines, finished products)

Nueve dominios que no pertenecen a ninguna empresa y comparten las reglas de abajo — con la salvedad de que `Client`, `ShippingLine` y `FinishedProduct` rompen dos: no se dan de baja con un `status` booleano sino con `SoftDeletes` real, y son los únicos sin ninguna ruta fija antes del `apiResource`.
...
### Finished Products (SPEC 36)

Catálogo de SKUs de productos terminados de un cliente. **No tiene ninguna relación con `Product`** (SPEC 07): comparten la palabra, no el dominio. Quinto dominio con `SoftDeletes` y **calco de `Client` en todo lo demás** —cinco rutas sin ninguna fija, `withTrashed()` interno, 404 al leer un borrado y 400 al escribirlo («El producto terminado ya fue eliminado»), `code` único global que el borrado no libera (duplicado **400 desde el service**, sin regla `unique` en el FormRequest; `regex:/^\S+$/u` → 422), `search` agrupado sobre `code` y `name`, orden `id ASC`, paginación opt-in `[10, 100]`, `deletedAt` con fecha solo en la respuesta del `DELETE`—, con estas diferencias:

- Tabla `finished_products`: `code` (`varchar(15)`, único), `name` (**sin índice único**), `presentation` y `boxes_per_pallet` (`decimal(10,2)`, `min:0.01|max:99999999.99`, sin unidad), `client_id` (índice, sin `cascade`), `registered_by`, `timestamps`, `deleted_at`. `casts()` `decimal:2` en los dos numéricos.
- **Roles propios**: lectura `role:` + `UserRole::allExcept(UserRole::Pilot)` —el `pilot` es 403 y `user`/`shipment` sí leen, al revés que el resto de catálogos—; escritura `role:administrator,export`. Sin `carrier.required`.
- **`name` solo a MAYÚSCULAS** (`FinishedProduct::normalizeName()` = `mb_strtoupper()` a secas): ni trim ni colapso, y dos SKU pueden llamarse igual. Para que por HTTP sea verdad, `bootstrap/app.php` **excluye las rutas `api/finished-products*` del middleware global `TrimStrings`** (`trimStrings(except: [closure])`) —la única ruta del proyecto que lo salta—; el `code` lo sigue recortando `normalizeCode()` y el `search`, el service. Un `name` de solo espacios sigue siendo 422 (`required` ignora el blanco).
- **`clientId` obligatorio y editable**: inexistente 422 (`exists:`), **borrado 400 «El cliente seleccionado ya fue eliminado»** desde `ensureClientIsActive()`, **también en un `PATCH` que reenvía el mismo cliente**. `client()` lee `withTrashed()`, así que `clientName` sobrevive al borrado del cliente, y `ClientService::destroy()` **no** se bloquea por productos terminados (`Client` no gana `finishedProducts()`).
- Filtro tolerante extra `clientId` (`FILTER_VALIDATE_INT`, no numérico se ignora). `FinishedProductResource` con **11 claves**: `id`, `code`, `name`, `presentation`, `boxesPerPallet` (string de dos decimales), `clientId`, `clientName`, `registeredByName`, `createdAt`, `updatedAt`, `deletedAt`.
- Contrato de cinco métodos con nombres largos (`getFinishedProducts`, `getFinishedProductById`, `createFinishedProduct(array, User)`, `updateFinishedProduct`, `deleteFinishedProduct`). Sin tool del asistente, sin exportación, sin relación con viajes.
...
- **`GET /api/trips/{trip}` pinta 43 claves, no 42** (cambio posterior a SPEC 33, sin spec): `TripController::show` hace `$found->load('positions')` y `TripResource` añade al final `positions` —el rastro entero de `trip_positions` como **pares `[lat, lng]` numéricos, la misma forma que `points`** (no pasa por `TripPositionResource`; floats con la precisión completa de la tabla, no los cinco decimales de `traveledPoints`, y disponible ya en `in_route`), orden `recorded_at asc, id asc`, **sin paginar**, `[]` si no hay puntos— vía `whenLoaded()`. **Excepto para el `pilot`**, al que no se le carga (emite el rastro pero no lo lee, regla de SPEC 26) y ve las 42. Los otros seis endpoints del `TripResource`, el listado, `/trips/current`, la tool `trip` del asistente (que usa `getTripById()` sin cargar el rastro) y `ReportService` no traen la clave. `getTripById()` **no** carga `positions`: lo consumen `TripPosition`, `TripFuel`, `TripExpense`, `TripTimeout`, `TripCost` y el canal, y arrastrar el rastro ahí sería pagar miles de filas para nada. Tests: `tripDetailKeys()` en `TripTest` = `tripResourceKeys()` + `positions`.
...
- **Se escribe solo en `/finish`**: `encodeTraveledRoute()` lee `TripPosition::query()` (solo `latitude`/`longitude`, orden `recorded_at asc, id asc`, **tal cual**: sin colapsar repetidos ni simplificar) y mete `traveled_polyline` en el **mismo `update()`** que `end_date` y `status`, sin `DB::transaction` nueva. No usa `Trip::positions()` —que llegó después, solo para el detalle— sino la consulta directa (precedente de `closeOpenTimeout()` sobre `TripTimeout`). El `POST /{trip}/positions` —la ruta más caliente— no cambia una línea; mientras el viaje está `in_route` el rastro se lee por websocket o `GET /{trip}/positions`.
...
Dominio `TripFinishedProduct` con tabla propia `trip_finished_products`: cuántas cajas de cada producto terminado del cliente (SPEC 36) lleva un viaje. **No se llama `trip_products`** para no confundirse con `Product` (SPEC 07).

- Tabla: `trip_id`, `finished_product_id` (FKs sin `cascade`, inmutables), `boxes` (`integer`, `>= 1`, el único editable), `registered_by`, `timestamps`. **Índice único `(trip_id, finished_product_id)`**, sin `deleted_at` ni `status`: el borrado es físico. `TripFinishedProduct::finishedProduct()` lee **`withTrashed()`** —un SKU borrado después sigue saliendo en la línea—. `Trip` gana `finishedProducts()` **solo para dos guardas**: ningún Resource del viaje la expone y `TripResource` (42/43) y `TripListResource` (19) no cambian de forma. La factory crea viaje y producto **del mismo cliente**.
- **Cuatro rutas sin anidar** en `routes/trip-finished-products.php` (decisión explícita, precedente `vehicle-expenses` de SPEC 14): `GET /api/trip-finished-products?tripId=` con `jwt.auth` a secas; `POST /`, `PATCH /{tripFinishedProduct}` y `DELETE /{tripFinishedProduct}` con `role:administrator,export`. `TripFinishedProductService` inyecta `TripServiceInterface` por constructor.
- **Listado**: `tripId` obligatorio (`IndexTripFinishedProductRequest`, sin él 422); inexistente **o borrado** 404 desde el service, no `exists:`. Ámbito de `getTripById()` —**incluido el `pilot` asignado**, `user` y `shipment`; el `carrier` fuera de su empresa, 403—. **Nunca pagina** (`?limit=` se ignora), orden `id ASC`, sin filtros; un viaje anterior a la spec lista `[]`. Número fijo de consultas (eager loading de `finishedProduct` y `registeredBy`), comprobado con `DB::getQueryLog()`.
- **Toda escritura de líneas exige el viaje `pending`**. Cinco guardas del `POST`, en orden y cada una 400: viaje borrado («El viaje ya fue eliminado») → no `pending` («Solo se pueden modificar los productos de un viaje pendiente») → producto borrado («El producto terminado seleccionado ya fue eliminado», calco de `ensureClientIsActive()`) → otro cliente («El producto terminado no pertenece al cliente del viaje») → repetido («El producto terminado ya está en el viaje»; el índice único lo respalda y el service traduce la `UniqueConstraintViolationException` al mismo 400). `PATCH` **solo `boxes`** (cuerpo vacío 422, precedente SPEC 11; `tripId`/`finishedProductId` se ignoran; `registered_by` no se reescribe): 404 «La línea de producto no existe» → borrado → no `pending`. `DELETE`: los mismos más **400 «El viaje debe tener al menos un producto terminado»** al borrar la última, contando dentro de una transacción con `lockForUpdate` sobre las líneas del viaje.
- `boxes` es `required|integer|min:1|max:999999` en los tres sitios (el tope evita el 500 de Postgres). `TripFinishedProductResource` con **diez claves**: `id`, `tripId`, `finishedProductId`, `code`, `name`, `presentation`/`boxesPerPallet` (string de dos decimales, **leídos en vivo** del producto, sin copia), `boxes` (entero), `registeredByName`, `createdAt` (`d-m-Y h:i:s A`).
- **Modifica `POST /api/trips`**: gana `products: [{ finishedProductId, boxes }]` (`required|array|min:1`, `finishedProductId` `distinct` → repetido 422) y **pasa de 14 a 15 campos obligatorios**, cambio incompatible sin periodo de gracia. `TripService::create()` corre ahora en `DB::transaction`: tras `ensureCatalogsAreUsable()`, `ensureFinishedProductsAreUsable()` aplica por línea las guardas de producto borrado y de cliente (una sola consulta a `finished_products`), y las líneas se escriben con `TripFinishedProduct::query()->create()` —**sin inyectar el contrato nuevo**, que cerraría un ciclo en el contenedor (precedente `closeOpenTimeout()`)—. Una línea rechazada no crea ni el viaje.
- **Modifica `PATCH /api/trips/{trip}`**: `products` se ignora en silencio (no está en `UPDATABLE_FIELDS`) y cambiar `clientId` en un viaje con líneas es **400 «No se puede cambiar el cliente de un viaje con productos terminados»**, después de revalidar catálogos; reenviar el mismo `clientId` no es cambio.
- Fuera: backfill, `products`/`totalBoxes` en los Resources del viaje, tarimas, pesos, guardas en `DELETE /finished-products` o `/clients`, bitácora de ediciones, tool del asistente, export, filtros de viajes por producto, push, paginación y listado global sin `tripId`. El hueco de SPEC 24 (el `PATCH` del administrador devuelve un `finished` a `pending`) reabre la edición de líneas.
...
- **N+1 resuelto sin relaciones nuevas**: `currentTrip`, `lastPosition` y `openTimeout` salen con **una consulta por conjunto** indexada por id en PHP y colgada del modelo como atributo transitorio (`setAttribute`), para que el Resource no consulte nada. `Vehicle` no gana `trips()` y `Trip` no gana `timeouts()` (línea de SPEC 26–28); `Trip::positions()` existe desde después pero el tablero no la usa. La última posición usa **`DISTINCT ON (trip_id)`** de Postgres, aislado en `latestPositionsByTrip()`; el proyecto ya está atado a Postgres por PostGIS. El Feature test comprueba con `DB::getQueryLog()` que el número de consultas no depende de N.
...
- **Reportes Excel (`Report`)**: la única salida del asistente que no es texto. El SDK solo admite **string** como resultado de tool (`InvokesTools::executeTool()` hace `(string) $result`) y el protocolo Vercel no tiene parts de archivo, así que `ReportService` genera el `.xlsx` **en el servidor**, lo sube con `FileStorageServiceInterface::store()` a `reports/{uuid}.xlsx` (público y permanente, como facturas y DPI) y la tool devuelve `{ fileName, url, rows, total, truncated }` (+ `totalAmount` en gastos); el modelo lo cita como enlace Markdown y el front puede leerlo también del `tool-output-available`. `exportTrips()` y `exportVehicleExpenses()` llaman al **mismo método del service que el endpoint, sin `limit`** (`Collection` completa, ámbito por rol intacto), pasan las filas por `TripListResource`/`VehicleExpenseResource` y traducen solo `status`, `nature` e `isInvoiced` al español; importes y estimaciones van como número para que Excel sume. **Tope `ReportService::MAX_ROWS = 5000`** (`take()` sobre la colección; `total` cuenta todo y `truncated` avisa), inyectable por constructor solo para que el test lo baje. `SpreadsheetWriterInterface` → `OpenSpoutSpreadsheetWriter` es el **único archivo que menciona `OpenSpout\`** (regla espejo de `Intervention\`): escribe en un `tempnam()` porque ZipArchive exige ruta, devuelve bytes y lo borra en `finally`. `fileName` (`viajes-{Y-m-d-His}.xlsx`, `gastos-vehiculo-{id}-…`) es **solo para mostrar**: la key es uuid y el navegador descarga `{uuid}.xlsx`. Sin purga, sin URL firmada y sin `Content-Disposition`. El prompt limita el uso a peticiones explícitas de archivo/Excel/reporte/exportar. La tool **no** usa el endpoint descargable de SPEC 38 ni su matriz por rol (17 columnas propias, sin cliente ni productos): alinearlos es deuda declarada para otra spec.

## Reporte de viajes descargable (SPEC 38)

`GET /api/reports/trips?dateFrom=&dateTo=` devuelve **en binario** un `.xlsx` con los viajes cuya `recolection_date` cae en el rango. **Única respuesta de la API que no es ni el sobre JSON ni un stream**: en éxito el cuerpo es el archivo (`response($contents, 200, [...])`) con `Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` y `Content-Disposition: attachment; filename="viajes-{dateFrom}_{dateTo}.xlsx"`; los errores (400, 401, 403) salen por `ResponseHandler` y el 422 con el formato de validación de Laravel. Sin tablas, columnas ni migraciones.

- `routes/reports.php` (prefijo `reports`, `name('reports.')`): `jwt.auth` + `role:` `UserRole::allExcept(UserRole::Pilot)` —el `pilot` es 403 desde el middleware— y **sin `carrier.required`**, como `GET /api/trips`.
- `ExportTripsReportRequest`: `dateFrom`/`dateTo` `required|date_format:Y-m-d`, `dateTo` `after_or_equal:dateFrom`, **sin rango máximo**. Los demás filtros de `GET /api/trips` (`status`, `clientId`, `shippingLineId`, `locationId`, `pilotId`, `vehicleId`, `search`) son opcionales y tolerantes, los lee el controller como string y no se validan; `limit` no se lee.
- `ReportServiceInterface::downloadTrips(User, filters): array{fileName, contents}`: llama a `getTrips()` **sin `limit`** —ámbito, filtros por día completo y orden `recolection_date desc, id desc` exactamente los del listado— y **no toca `FileStorageServiceInterface`**: nada llega al bucket. `exportTrips()` y la tool no cambian.
- **Tope**: más de `MAX_ROWS` (5000, el mismo de la tool) es **400 «El reporte excede 5000 viajes; acota el rango de fechas»** antes de escribir; el mensaje cita la constante, no el `$maxRows` inyectado (los tests lo bajan con `app()->when(ReportService::class)->needs('$maxRows')`). El `count()` llega después de cargar la colección: riesgo aceptado. Un rango vacío es 200 con solo la fila de encabezados.
- **La fila se arma desde el modelo `Trip`**, no desde `TripListResource` (no trae cliente, destino final ni transporte): 22 columnas base (`TRIP_REPORT_HEADERS`) con estado en español (`STATUS_LABELS`), fechas `d-m-Y h:i:s A`, km/horas como `float` y `null` como celda vacía. Sobre la colección, `load('client')` —una consulta por relación, nunca por viaje—.
- **Matriz de columnas en el service, no en las rutas** (como `mileage` en SPEC 13): solo `PRODUCT_ROLES` (`administrator`, `manager`, `export`, `shipment`) cargan `finishedProducts` (orden `id`) + `finishedProducts.finishedProduct` (`withTrashed()`) y reciben al final `TRIP_PRODUCT_HEADERS`: «Productos» (`CODE × N cajas; CODE × N cajas`, vacía sin líneas) y «Total de cajas» (`0` sin líneas). `carrier` y `user` quedan en 22 **aunque lean `GET /api/trip-finished-products`**: el reporte tiene su propia matriz.
- Fuera: dinero (galones, viáticos, costo), rango máximo, otros reportes, CSV/PDF, cola o archivo en el bucket.
